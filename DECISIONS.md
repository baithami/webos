...
## [2026-07-06] Full-app audit + hardening pass (shell, game, API)
Three parallel deep-dives (shell / game / auth+persistence) against the whole app; baseline tsc, lint, build, and verify:cases all clean. Fixed this pass:

- **P0 — unauthenticated APIs (separate commit):** /api/state, /api/reset, /api/media accepted any `?key=` from the internet. Now Bearer-token-gated via `src/lib/server/auth.ts`. Guest stays open by design; media GETs public.
- **P1 — marquee re-rendered the whole shell:** every pointermove during a rubber-band drag re-rendered Desktop → WindowLayer/Taskbar/every open window. First attempt moved selection into useUIStore — that HARD-FROZE real Chrome (infinite loop; renderer unresponsive, reproduced twice, bisected by rebuilding with/without the change; headless Chromium never reproduced it because the guest boot screen never finishes under headless timer throttling, so no Playwright run ever reached the desktop). Final design keeps the drag entirely outside React: refs + direct DOM writes for the box, `marquee-hit` class toggling (globals.css) for icon highlights. Zero React work per pointermove, and the freeze sequence (drag → coordinate-click Start ×2) now passes in real Chrome.
- **Shell:** SystemLayer click-catcher z-index derives from Z constants (was hard-coded 750); deleted dead `desktop/Clock.tsx`; AppContent wraps every app in a Win95-styled error boundary ("This program has performed an illegal operation" + Restart button) so one crashing app can't blank the desktop.
- **Game (old audit P2s):** F-07 unknown case ids now report locked; F-08 persist config has version:1 + migrate hook; F-09 case-002 declares LIKE (harness WARN conditional → suite now ALL CHECKS PASSED, zero warnings); F-12 Inbox reader defaults to the active case when already opened; F-10 Inbox recolor (navy selected row #000080, navy unread dot, flat beveled navy avatar).
- **Ops note:** port 3000 runs the standalone production server — a running `next-server` keeps old route modules in memory, so API changes need `npm run build:standalone` + restart (mixed old/new behavior was observed mid-audit). Server restarted on this pass's build. Guest server state was reseeded (a curl test overwrote it); account state files untouched.

Known remaining (deliberate, next passes): login-time hydration can clobber seconds-old local progress (no version stamp on synced state); Supabase saves are fire-and-forget with no sync-status UI; media library is shared across accounts (quota/isolation); F-13 lucide-react → inline SVG shell-wide; game content wants cases 4-6 + an XP sink.

## [2026-07-07] Sync hardening + cases 4–6 + rank ladder
Backlog pass from the 2026-07-06 audit, in priority order.

- **Sync correctness (the two data-loss holes).** (1) StateSync treated a FAILED /api/state read like an empty server and would seed + save a fresh desktop over the account's real state on any network blip — now a failed account read retries with backoff (2s→30s) and blocks the debounced saver until a real load lands; guest still gets its localStorage offline fallback but never writes the seed back. (2) GameSync's loadState() fell back to this browser's localStorage on Supabase failure — which is GUEST (or another account's) progress, and would then be pushed up to the signed-in account. New `loadRemoteState()` returns null on failure instead; GameSync retries and keeps remote saves blocked. Bonus: hydration now merges progress made while the load was in flight (union additive fields, max hint counters, keep in-flight active case) instead of clobbering it. New `useSyncStore` + a red CloudOff tray indicator ("Offline — changes saved locally") driven by every save/load path.
- **Cases 4–6** (`case004–006.ts`), continuing the bureaucratic-absurdist ladder: #0004 The Phantom Parking Spot (3-table JOIN chain, beige-sedan witness detail, two red herrings — wrong-color car in the spot, right-color car elsewhere), #0005 The Coffee Fund Discrepancy (NOT IN + subquery, phantom run codes), #0006 The Midnight Shredder (finale: BETWEEN night window + JOIN + GROUP BY/HAVING count-of-nights pattern proof, one-night janitor red herring). `difficulty` widened to 1–5. Each has a Vane intro in supervisors.ts and hardcoded solution-path + red-herring assertions in verify-cases.js — suite: ALL CHECKS PASSED, zero warnings.
- **Rank ladder** (`ranks.ts`): CADET→CHIEF INSPECTOR in 100 XP steps — tops out exactly at 6 solved cases. Shown as "RANK: … · N XP" in the Inbox header (tooltip = next threshold) and as an engraved nameplate on the Supervisor Console control row. Cosmetic only, no gating.
- Verified in real Chrome against the rebuilt standalone server: Inbox lists six dispatches (4–6 locked), rank line renders, console shows the rank plate mid-intro.

## [2026-07-07] Sgt. Vane portrait art on the Supervisor Console
User-supplied portrait installed as `public/supervisors/vane/base.jpg`. FaceScreen now loads through a three-step cascade: per-expression talk frames (`<expr>_<open|closed>.png`, unchanged convention) → static `base.jpg` with a brightness talk-pulse + blink dim so the machine still reads as alive → CSS CRT placeholder. `USE_PLACEHOLDER_FACE` flipped to false (now a test-only override). No conversion tooling on the host (no ImageMagick/PIL/ffmpeg), so the JPEG ships as-is (215 KB, 768×1152) — fine under object-fit: contain on the black CRT. Verified in Chrome: portrait renders behind the scanline overlay during the case-002 intro.
